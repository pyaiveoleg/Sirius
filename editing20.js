...
function getCtrlValue(e) {
	...
	if(e.hasA("save_val")) {
		...
		pval = e.value;
	}  else if(e.tagName == "INPUT" && e.A("vtype")=='CH') {
		val = e.checked ? '1' : '0';
		pval = e.checked ? '1' : '0';
	} else if(e.tagName == "INPUT" &&
			({'2': 1, '3' : 1, '3M': 1, 'two': 1, 'three': 1, 'threeM': 1})[e.A("vtype")] ) {
		var d = {'РґР°':'1', '1':'1', '0':'0', 'РЅРµС‚':'0', '?':'', '':''};
		var t = {'РґР°':'РґР°', '1':'РґР°', '0':'РЅРµС‚', 'РЅРµС‚':'РЅРµС‚', '?':'', '':''};
	...
	if(src.A('add') == 'suspend') {
		...
		if(src.hasA("max_count") && ex) {
			...
			if(cnt >= (+src.A("max_count"))) {
				alert("Р‘РѕР»СЊС€Рµ "+src.A("max_count")+" РґРѕР±Р°РІРёС‚СЊ РЅРµР»СЊР·СЏ!");
				return;
			}
		}
...
function prepareControls(cnt, init) {
		...
		if(a[i].A('lobload')=='filer') {
			...
			ne.innerHTML =
					...
					+'<form method="POST" enctype="multipart/form-data" target='+framename+' action=/az/server/php/filer.php>'
						...
						+'<input type="file" name="file_data" value="Р¤Р°Р№Р»" onchange="onchooseFileForm(this)">'
					+'</form>'
					+'</div>';
			e.appendChild(ne);
	...
	for(var i = 0; i < a.length; ++i)
	{
		...
		if(e.tagName == "INPUT" && (e.A("vtype")=="3" || e.A("vtype")=="three")) {
			...
			ne.setA("bool-state", {'РґР°':'yes','1':'yes', 'РЅРµС‚':'no','0':'no','':'?','?':'?'}[ e.V() ]);
			ne.innerHTML =
				'<a yes href="" onclick="bool3state(this);return false">РґР°</a>/<a no href="" onclick="bool3state(this);return false">РЅРµС‚</a>';
			e.parentNode.insertBefore( ne, e);
		...
		if(e.tagName == "INPUT" && (e.A("vtype")=="2" || e.A("vtype")=="two") && e.type != 'hidden') {
			var ne = document.createElement("INPUT")
			ne.type = "checkbox";
			var v1 = e.A('checked-value')!=null && e.A('checked-value') || e.A("vtype")=="two" && 'РґР°' || '1';
			var v0 = e.A('unchecked-value')!=null && e.A('unchecked-value') || e.A("vtype")=="two" && 'РЅРµС‚' || '0';
			var checked = {'РґР°':true,'1':true}
...
function show3stateMenu(e) {
	...
	e.menu.className = "bc"+{'РґР°':1, 'РЅРµС‚':0,'1':1,'0':0,'':'','?':'' }[getCtrlValue(e).val];
	setBodyToContaner(e.menu,  "<div id=boolChoosew><table>"+
		...
		"<tr class=f><td><a href='javascript:undefined;' onclick='this.closeModal(0)'>РЅРµС‚</a>"+
		"<tr class=t><td><a href='javascript:undefined;' onclick='this.closeModal(1)'>РґР°</a>"+
		"</table></div>");
...
function keydown3stateMenu(ev, e) {
	...
	else if(ev.keyCode == 37) { e.setV('РЅРµС‚'); e.callchange(); }
	else if(ev.keyCode == 39) { e.setV('РґР°'); e.callchange(); }
...
function bool3state(o)
{
	...
	pn.nextSibling.value = {'yes':'РґР°','no':'РЅРµС‚','?':'?'}[state];
...
function checkThisCard(e, msg) {
	...
	if(msg === undefined) msg = "РћСЃРЅРѕРІРЅС‹Рµ С„РѕСЂРјР°Р»СЊРЅС‹Рµ РїСЂРѕРІРµСЂРєРё СѓСЃРїРµС€РЅРѕ РІС‹РїРѕР»РЅРµРЅС‹";
	if(!ind) alert("РџРѕР»СЏ С„РѕСЂРј СЃРѕРґРµСЂР¶Р°С‚ РЅРµРґРѕРїСѓСЃС‚РёРјС‹Рµ РґР°РЅРЅС‹Рµ. РџСЂРѕРІРµСЂСЊС‚Рµ РµС‰Рµ СЂР°Р·.")
	if(ind && msg)
		alert(msg)
	if(!ind) {
		var a = e.QSA('[error="Y"]');
...
var sureText = "Р’С‹ СѓРІРµСЂРµРЅС‹?"
...
function onchooseFileForm(e) {
	...
	if(e.files && e.files.length && c.A("max-size") != null ) {
		...
		if( max = c.A("max-size").match(/^\s*(\d+)\s*([mk])?/i) ) {
			if(+max[1]*{ "m":1024*1024, "k":1024, "": 1 }[max[2] && max[2].toLowerCase() || ""] < e.files[0].size ) {
				alert("Р Р°Р·РјРµСЂ С„Р°Р№Р»Р° РїСЂРµРІРѕСЃС…РѕРґРёС‚ "+c.A("max-size") );
				return;
			}
		} else
	...
	if(e.value.match(new RegExp('('+ft.join('|')+')$', 'i')) == null) {
		alert('РќРµ С‚РѕС‚ С‚РёРї С„Р°Р№Р»Р°: РЅСѓР¶РµРЅ '+c.A('filetypes'));
		return;
	}
	if(c.A('lobarg'))
...
function funtionalFilter(sp, params) {
	...
	function EXPLODE(list) {
		if (!list) return [];
		var listName = list.substr(1);
		var vals = params[listName].split(','), valNames = [];

		for (var i in vals) {
			valNames.push(listName + (+i + 1));
			params[listName + (+i + 1)] = vals[i];
		}
		delete params[listName];

		return valNames.map(function (e) { return "?" + e + ""; });
	}
...
function applyFuncFilter(e, offset, place, direct_txt) {
	...
	for(var i = -1; i < a.length; ++i) {
		...
		if(fc.A('filter_ctrl'))
			mode[ fc.A('filter_ctrl') ] =
				...
				fc.type==='select-multiple' ? getMultipleValues(fc) :
	...
	if(cond) {
		//---- order by
		cond.order_by = '';
		order_container = e.hasA('filter_for') && e.evalHere(e.A('filter_for')) || I('filtred');
		if (cond && order_container && order_container.hasA('order_by') && order_container.A('order_by')) {
			cond.order_by = ' ORDER BY ' + order_container.A('order_by');
		}
		//---
		if(e.QS('[filter_ctrl-replace]'))
			if(uri.URLParam('cmd'))
				cond.args.unshift('*'+cond.where + cond.order_by);
			else
				uri = uri.setURLParam('cmd', '*'+cond.where + cond.order_by);
		else
			if(uri.URLParam('cmd'))
				cond.args.unshift('*'+cond.where + cond.order_by);
			else
...
var Calendar = (function() {
	...
	function calendarElement(cont, input, D, onOpen, onClose, border) {
		...
		var month_names = [ "РЇРЅРІР°СЂСЊ", "Р¤РµРІСЂР°Р»СЊ", "РњР°СЂС‚", "РђРїСЂРµР»СЊ", "РњР°Р№", "РСЋРЅСЊ", "РСЋР»СЊ", "РђРІРіСѓСЃС‚", "РЎРµРЅС‚СЏР±СЂСЊ", "РћРєС‚СЏР±СЂСЊ", "РќРѕСЏР±СЂСЊ", "Р”РµРєР°Р±СЂСЊ" ];
...
function setSortField(e, field) {
	var order = ' ASC';
	if (I('filtred') && I('filtred').hasA('order_by') && I('filtred').A('order_by').match(/.+ ASC$/)) {
		order = ' DESC';
	}



	I('filtred').setA('order_by', field + order);
	applyFuncFilterT(QS('[filter_ctrl]'));
}
